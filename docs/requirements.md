...
**核心差异化**: CRDT 多端同步 + MVCC 角色分支 + 意图图谱导航 + Token→Action Bridge 具身人格移植 + 酒馆式混合格式人格工程 + **Anthropic 认知仿生记忆架构借鉴**
...
- **认知仿生记忆架构（Anthropic 借鉴）**：工作/情景/语义三层 + 差异化遗忘 + Dreaming 主动反思 + 重要性评分驱动固化
...
**重要性加权公式**：
```python
score = cosine_similarity(query_vec, memory_vec) * importance * (1 + log1p(access_count))
```
- `importance` 默认 0.5，可由 Reflection Agent 或用户显式调整
- `access_count` 提供频率提升，避免冷记忆被完全淹没
- 该公式在 `SimpleEpisodicStore` 和 `FaissEpisodicStore` 中统一实现
