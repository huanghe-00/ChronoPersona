...
本文档用于系统级技术交流与架构评审。为建立可信的技术讨论基线，文档中所有功能均按以下三级状态如实标注：

- **[已实现]**：代码已真实落地，通过单元测试或集成测试验证，可在 MVA 环境中直接运行。
- **[接口冻结]**：抽象接口、数据 Schema、Mock 实现已完成，但真实后端（如生产级模型、数据库、外部服务）尚未接入，当前以 Mock 或空函数运行。
- **[远期规划]**：架构设计已完成，需求与接口已识别，但未进入当前 MVA 代码基线。

**核心差异化不是依赖更大的 LLM，而是通过分布式系统与数据库领域的成熟抽象重构记忆层**：自研轻量 LWW-CRDT 保证多端最终一致性，MVCC `branch_id` 物理隔离实现角色硬隔离，意图图谱将检索从"相似度匹配"升级为"结构化认知推理"。
...
| 差异化维度 | 传统方案现状 | ChronoPersona 方案 |
|-----------|-------------|-------------------|
| **多端一致性** | 单节点架构，各自为政 | **[已实现]** 自研 LWW-CRDT + HLC 混合逻辑时钟，冲突保留不覆盖 |
| **角色隔离** | Prompt 替换，记忆共享同一向量空间 | **[已实现]** MVCC `branch_id` 物理隔离，切换人格等效于 `git checkout` |
| **检索语义** | 纯向量相似度匹配 | **[已实现]** 意图图谱导航（8 类语义边 + 8 类意图策略），覆盖时序/因果/指代 |
| **人格工程** | 自由文本 System Prompt | **[已实现]** 四层混合格式 Anchor + 风格指纹漂移检测 |

### 1.3 技术评审展示边界

MVA 阶段的核心目标是**验证架构分层与接口契约的正确性**，而非堆叠功能数量。以下能力已真实落地并可演示：
- L0–L3 记忆分层端到端数据流；
- CRDT 多端冲突保留与 add-wins 语义；
- 纯 Python 状态机 Agent Core（Input → Intent → Memory → LLM → Output）；
- GridWorld 具身感知与 Token→Action Bridge。

以下能力处于 **[接口冻结]** 或 **[远期规划]** 状态，将在文档后续章节明确标注：
- 真实 LLM 模型路由（Qwen3.5/DeepSeek/Kimi）；
- PostgreSQL CTE 持久化图谱；
- WebSocket 真实多端组网；
- LSTM 生产级训练闭环。
...
以下决策是架构评审的核心讨论点，每一项均伴随明确的取舍与反取舍。

#### 决策一：自研 KV 级 CRDT，而非采用 Yjs/Automerge

- **问题**：Yjs 是面向富文本协同的字符级 CRDT，其合并语义针对文本编辑器的插入/删除优化。
- **我们的场景**：L0 层是结构化 KV 存储（用户画像、活跃偏好、情感状态），需要精确控制 HLC 比较逻辑、嵌入 500ms clock-skew 检测与冲突标记。
- **取舍**：自研 `LWWMap` 代码量 < 500 行，但完全掌控 add-wins 语义与 HLC 逻辑时钟；代价是放弃 Yjs 成熟的生态与文本合并能力（而我们不需要文本合并）。
- **状态**：[已实现]

#### 决策二：MVCC 物理隔离，而非逻辑过滤

- **问题**：角色隔离若仅靠向量库的 metadata 过滤，存在过滤条件被绕过、Prompt 注入导致串台的风险。
- **方案**：所有数据操作显式传递 `branch_id`，L1/L2/L3 按分支物理隔离；跨分支共享仅通过显式 `merge`，且需经过 `IPrivacyFilter` + `IRelevanceFilter` 双重过滤。
- **取舍**：绝对避免串台，但增加了跨分支查询的复杂度（无法单条 SQL 跨分支 JOIN）。MVA 通过 Mock 占位验证过滤接口。
- **状态**：物理隔离机制 [已实现]；跨分支隐私过滤器 [接口冻结]

#### 决策三：意图图谱导航，而非纯向量 RAG

- **问题**：纯向量 RAG 对"后来怎样"（时序）、"为什么焦虑"（因果）、"那个方案"（指代消解）的召回依赖向量相似度，精度不足。
- **方案**：L3 语义记忆采用节点+边+属性的图模型，8 类语义边将检索升级为结构化认知推理。
- **取舍**：图模型的构建需要额外的 Reflection Agent 异步流水线（实体提取 → 边构建 → 冲突检测），增加了系统复杂度和延迟；MVA 阶段为纯内存 BFS（`collections.deque`），P99 < 10ms（千节点规模），但进程重启丢失图谱。
- **状态**：内存 IntentGraph + Navigator [已实现]；PostgreSQL CTE 持久化 [远期规划]

#### 决策四：T0 规则引擎 + LSTM 回归，而非端到端 LLM 做情感分类

- **问题**：LLM 做情感分类会因 Prompt 变化、温度参数产生漂移，且黑盒 reasoning 难以调试。
- **方案**：T0 规则层（关键词匹配状态机，<1ms，100% 可预期）作为默认路径；LSTM 监督回归器作为可训练插槽，捕捉"连续多轮负面输入"等时序模式。
- **取舍**：确定性高、可审计、资源效率高；但规则引擎覆盖边界有限，LSTM 需要标注数据。明确禁止在情感回归层引入 PPO/GRPO（违反 AIDER.md PLACEHOLDER 红线，且任务类型错配）。
- **状态**：T0 规则引擎 [已实现]；LSTM 训练骨架 [接口冻结]；RL 优化 [远期规划]

#### 决策五：先内存 BFS 验证语义，再上 PostgreSQL CTE

- **问题**：MVA 时间有限，是否需要直接引入 PostgreSQL 持久化？
- **方案**：MVA 核心目标是**验证意图图谱的检索语义正确性**（A6 场景 Recall@5 对比纯向量的提升）。千节点规模下 Python `deque` BFS 延迟 < 10ms，足够支撑评估。Schema 与 CTE 查询模板已设计冻结，W8+ 生产级再迁移。
- **取舍**：快速验证、零 Docker 依赖；代价是进程重启丢失数据、无数据库执行计划优化。
- **状态**：内存 BFS [已实现]；PostgreSQL CTE 持久化 [接口冻结，Schema就绪]

#### 决策六：结构化操作原语，禁止文本级 diff/merge

- **借鉴 Cursor 多 Agent 架构的核心洞察**：代码是高度结构化的 AST，文本级 CRDT 合并结构化数据会导致"语法崩溃"。
- **方案**：L3 语义记忆强制使用节点级操作原语（AddConcept/LinkEntities/DeprecateConcept），禁止文本级 diff/merge。多端冲突时保留双版本并标记 `CONTRADICTS` 边，由 LLM 仲裁或用户确认。
- **状态**：[已实现]
...
**Q: 简历/项目介绍中常见的夸大风险点有哪些？你们如何规避？**

**A:** 我们在技术文档中主动识别并标注了三项易被夸大的模块：
1. **T0–T7 模型路由**：路由策略与降级链已设计冻结，但**真实模型接入（本地 Qwen3.5 + 云端 DeepSeek/Kimi）尚未实现**，MVA 阶段用 Mock 验证接口契约。
2. **SQLite 缓存与三级风险兜底**：策略层已定义，执行层为 Mock，需 W8+ 补齐异常捕获与安全回复固化。
3. **四层 Persona Anchor**：Schema 与 Prompt 注入链路已跑通，但 W++ 专用语法解析器、权限执行的策略引擎属于接口冻结 + Mock 占位状态。

在技术交流中，我们的表述原则是：**已真实落地的如实描述，框架/接口就绪但无真实后端连接的明确标注，规划中的不夸大。**
...
**结论**：四层混合格式的**数据模型与 Prompt 注入链路**已在 MVA 跑通；但 W++ 专用语法解析器、权限执行的策略引擎属于**接口冻结 + Mock 占位**状态。可表述为"四层 Schema 已冻结并验证注入效果，专用解析器与权限策略引擎是 W8+ 生产级收尾项"。
...
**结论**：T0–T7 的**分层策略、降级链、成本追踪 Schema** 已在 MVA 完成设计冻结；但**真实模型接入（本地 Qwen3.5 + 云端 DeepSeek/Kimi）尚未实现**。MVA 的核心目标是验证"Agent Core 在接到不同 task_type 时能否正确路由到对应模型槽位"，而非真实跑通多模型推理。应明确表述为"路由策略与降级链已设计冻结，真实模型接入是生产级部署项"。
...
| 三级兜底 | 中 | 一级降级（模型切换）可推迟；但二级兜底（本地规则引擎）已部分实现（T0/T1 规则），这本身是 Agent Core 正确性的一部分。三级安全回复建议 W8 前补全，防止对外技术演示时异常路径暴露实现粗糙。 |

**结论**：SQLite 缓存属于**生产级成本优化项**，MVA 不做不影响架构验证；三级风险兜底的**策略层已定义，执行层为 Mock**，可表述为"降级链与兜底策略已设计，生产环境需补齐异常捕获与安全回复固化"。

---

## 14. 技术表述修正建议

基于上述实现状态，以下是针对这三项技术的**诚实且专业**的表述：
