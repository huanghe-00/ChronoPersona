...
class TestIntentGraph:
    ...
    def test_add_concept_branch_isolation(self) -> None:
        ...
        graph = IntentGraph()
        graph.add_concept(Concept("c1", "概念1", "abstract", branch_id="b1"), branch_id="b1")
    ...
    def test_add_edge_validates_type(self) -> None:
        ...
        with pytest.raises(ValueError):
            graph.add_edge(SemanticEdge("e1", "a", "b", "INVALID", branch_id="main"), branch_id="main")

    def test_navigate_respects_max_hops(self) -> None:
        """BFS navigation respects max_hops limit."""
        graph = IntentGraph()
        graph.add_concept(Concept("c1", "起点", "abstract", branch_id="main"), branch_id="main")
        graph.add_concept(Concept("c2", "中间", "abstract", branch_id="main"), branch_id="main")
        graph.add_concept(Concept("c3", "终点", "abstract", branch_id="main"), branch_id="main")
        graph.add_edge(SemanticEdge("e1", "c1", "c2", "IS_A", branch_id="main"), branch_id="main")
        graph.add_edge(SemanticEdge("e2", "c2", "c3", "IS_A", branch_id="main"), branch_id="main")
